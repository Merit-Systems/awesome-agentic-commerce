...
- [Hawker](https://hawker-web.vercel.app) - Turn any API into an agent-payable product: paste an OpenAPI spec, get a hosted MCP endpoint agents discover, call, and pay per use. Dual rail (x402 USDC + Stripe prepaid credits), upstream keys encrypted, and each product shows *measured* success-rate/latency/paid-call-count (observed by the gateway, not seller-claimed). Failed calls are never charged. First products are Korean government data (apartment prices, business-registration checks, holidays). Open source (AGPL). [Code](https://github.com/HyosikPark/Hawker)
