...
- [wayleave](https://github.com/gibrancorbin11-hub/wayleave) - Node/Express middleware that gates and prices agent traffic rather than settling it. Verifies Web Bot Auth signatures (RFC 9421 profile, Ed25519) against key directories you configure, sorts requests into verified/declared/suspected/human lanes, and returns an x402-shaped 402 with a price on chosen routes. Humans never see a paywall. Zero dependencies. Settlement is a pluggable stub — the challenge and payment-proof check work, no payment network is wired. MIT. ([npm](https://www.npmjs.com/package/wayleave))
