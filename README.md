...
- [almanac (Celestial Almanac)](https://almanac.locomot.io) - Live x402 suite of *verifiable* facts and trust primitives for agents, settling USDC on Base. Deterministic truth endpoints (`/sky` ephemeris, `/field` magnetic declination, `/time`, `/prosody`) each return a re-executable, EIP-191-signed `attestation` so answers self-certify — reproduce and check, don't just trust. A trust wing adds `/random` (provably-fair drand draw), `/timestamp` (drand proof-of-existence), `/sealed` (timelock encryption), and a sealed-bid **auction house** (`/auction/*`) where the auctioneer can't see the bids until they open. Open-source compute core: [`almanac-compute`](https://pypi.org/project/almanac-compute/) (PyPI + MCP registry, MIT), proofs verified against NOAA/JPL.
