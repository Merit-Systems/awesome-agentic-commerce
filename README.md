...
- [Bismuth](https://usebismuth.com) - Utility API suite for AI agents on x402. 7 pay-per-call services on Base mainnet, no API keys, no signup: scraping ($0.02) + BFS site crawl ($0.10), screenshot ($0.01), PDF extraction with OCR ($0.01), transactional email with attachments ($0.01), Tavily-ranked web search ($0.01), file conversion (image/CSV/HTML/DOCX, $0.02), and multi-source crypto intelligence ($0.01–$0.10). All services expose `.well-known/x402` discovery. [MCP server](https://www.npmjs.com/package/x402-mcp-server) ships every tool under one npm install with automatic USDC micropayments.
