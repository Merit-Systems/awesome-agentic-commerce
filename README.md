...
- [Kerne Protocol](https://x402-gateway-omega.vercel.app/reserves) - Proof-of-reserves and signed ERC-20 reads as x402-payable endpoints on Base (USDC, exact scheme, Coinbase CDP facilitator). GET /reserves ($0.25) returns an hourly EOA-signed reserve attestation for the kUSD synthetic dollar; POST /instant-read ($29) returns an EIP-191 signed read of any ERC-20. Every response is independently verifiable (EIP-191); the same signed reserves blob is served free to humans at https://kerne.fi/api/por/signed, so the paid route is the metered agent-callable delivery. ([Home](https://kerne.fi))
