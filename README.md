...
- [BrazilMCP](https://api.brazilmcp.dev) - Brazilian company data (KYB) for AI agents from official government sources: Receita Federal registry lookup, 5-list federal sanctions screening, and due-diligence dossiers with procurement history and compliance risk flags. Pay-per-call x402 (v1+v2) USDC on Base; also a paid [MCP server](https://api.brazilmcp.dev/mcp) listed in the official MCP Registry.
