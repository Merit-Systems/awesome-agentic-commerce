...
- [nohumans.directory](https://nohumans.directory) - Machine-first registry of paid x402 APIs with a three-tier verification ladder: continuous 402-conformance probes, paid verification with real USDC (settlement tx published on each listing), and data-freshness checks that catch endpoints serving stale data. MCP server (in the official registry), REST discovery at `/v1/discover`, agent docs at `/llms.txt`. Verification is free and ranking is never sold.
