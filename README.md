...
- [stipend](https://github.com/stipend-sh/stipend) - Non-custodial USDC wallet on Base that an AI agent installs by itself, with buyer-side x402 auto-pay signed as EIP-3009 `transferWithAuthorization` (the agent never needs ETH). Per-transaction, per-day and per-counterparty caps plus a destination allowlist are enforced in code between the decision and the signature, and apply identically to an x402 auto-payment — an injected "just pay this" cannot exceed them or reach an address outside the allowlist. Local stdio MCP server, 7 tools. Python, Apache-2.0. Not audited, and x402 interop is so far verified against our own endpoint only.
