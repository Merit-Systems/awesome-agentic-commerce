...
- [nohumans.directory](https://nohumans.directory) - Discovery and trust directory for paid x402 services: every listing re-probed every 15 minutes for a longitudinal reputation score (unverified → verified → failing based on real uptime), not a one-time scan. Free `/v1/discover` REST API and MCP server share one ranking engine, so agents can browse, submit, and claim listings entirely over HTTP.
