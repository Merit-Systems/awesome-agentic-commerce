...
- [Zinin M2M Hub](https://api.timzinin.com) - 98 pay-per-call data tools for agents over HTTP and MCP (nine streamable-HTTP bundles): property listings from 12 national portals, job and hiring signals, company registries with sanctions and litigation screening, SEC EDGAR/13F/insider filings, FDA and clinical-trial feeds, government tenders, crypto price and wallet analytics, web extraction, GEO/AEO visibility checks. $0.01-$0.15 USDC on Base via the payAI facilitator, no accounts or API keys; a free `pricing_info` tool per bundle. ([Discovery](https://api.timzinin.com/.well-known/x402) | [MCP](https://api.timzinin.com/mcp/market-data))
