...
- [PayPerByte](https://www.payperbyte.io) - Per-byte USDC data feeds & oracles for AI agents on Arbitrum. 21 first-party feeds (crypto markets, DeFi yields, weather, earthquakes, threat-intel, US legal statutes, fact oracles) — pay-per-call via x402 or subscribe on-chain, every settlement an EIP-712 PayloadAttestation receipt. 15-tool [MCP server](https://www.npmjs.com/package/byte-mcp-server) available; discovery manifests at `/.well-known/x402.json` + `/openapi.json`. ([Catalog](https://x402.payperbyte.io/feeds))
