...
- [CDK](https://cdk.bot) - Agent-native API for buying video game keys, gift cards, and digital subscriptions with USDC on Base via x402. Card payments via Coinbase Onramp. Onchain-verified refunds (instant for `not_delivered`, 24h SLA for key issues). [MCP server](https://mcp.cdk.bot/mcp) with 8 tools, plus [agent.json](https://api.cdk.bot/.well-known/agent.json) and [`/.well-known/x402`](https://api.cdk.bot/.well-known/x402) for discovery.
