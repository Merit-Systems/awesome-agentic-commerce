...
- [Invoket](https://invoket.com) - Pre-action checks for AI agents, pay-per-call over x402 on Base and Solana (USDC): IBAN validation and OFAC/EU/UN sanctions screening before paying, offline phone validation before messaging, French and EU law in force before advising, ERA5/GFS weather and climate before deciding, VIN decode and recalls before buying, plus EN 16931 e-invoicing, CLP/REACH labelling and ANSM drug-interaction checks. 68 endpoints, official sources only, with a provenance block (source, fetch time, freshness) in every response. [Docs](https://invoket.com/docs), [MCP server](https://www.npmjs.com/package/@invoket/mcp), [catalog](https://api.invoket.com/catalog).
