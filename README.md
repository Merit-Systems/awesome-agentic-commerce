...
- [chipscan](https://chipscan.xyz) - Meme-token due diligence API for trading agents: security audit ($0.01/call) and chip-structure lifecycle analysis ($0.05/call) on Solana, BSC, Base, and Ethereum. Pay-per-call USDC via x402 on Base; OpenAPI + `/.well-known/x402` discovery. ([manifest](https://api.chipscan.xyz/))
