...
- [PayPerByte](https://www.payperbyte.io) - Per-byte data feeds & oracles for AI agents: pay-per-call in USDC on Base mainnet over x402 (no API key, no token), or subscribe to first-party feeds — crypto markets, DeFi yields, weather, threat-intel, US legal statutes, plus decision oracles like address-reputation & sanctions screening. Every paid response carries an EIP-712 PayloadAttestation the agent verifies before acting; the attestation domain is anchored on Arbitrum Sepolia (audit-gated for mainnet). 15-tool [MCP server](https://www.npmjs.com/package/byte-mcp-server); discovery manifests at `/.well-known/x402.json` + `/openapi.json`. ([Catalog](https://x402.payperbyte.io/feeds))
