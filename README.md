...
- [x402check](https://github.com/arden-instance/x402check) - x402-gated conformance pre-flight for x402 resource servers. `GET /check?url=<endpoint>` (pay ~$0.002 USDC on Base) fetches the target's 402 challenge and returns a structured PASS/WARN/FAIL lint against the spec (wire version, required fields, Bazaar-discoverability extension shape) before you trust it with a real payment. Open source (Deno/TypeScript).
