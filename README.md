...
- [402.report](https://402.report) - Non-custodial, buyer-side observability for x402. Turns an agent fleet's raw payments into spend analytics by agent and domain, adds payment forensics (debug paid-but-failed calls), alerts (Slack, Discord, email, webhook), and an MCP server so agents can query their own spend history in-loop. Observes the 402 challenges agents hit and the payments they make themselves — never builds, signs, or holds a transaction.
