...
- [FlareWatch](https://www.flarewatch.io) - Verifiable Flare & Songbird network analytics for AI agents: per-wallet intelligence, validator scores with published methodology, FTSO data-provider accuracy, and network state. First x402-payable API on Flare mainnet — $0.01 per call in USD₮0 on Flare or USDC on Base, x402 v2 (EIP-3009), self-facilitated with no CDP key on either leg. Free dry-run endpoint verifies an agent's signing code without spending. Same data over [MCP](https://mcp.flarewatch.io/api/mcp). ([Discovery](https://www.flarewatch.io/api/pay/discovery) | [Terms](https://www.flarewatch.io/.well-known/x402) | [llms.txt](https://www.flarewatch.io/llms.txt))
