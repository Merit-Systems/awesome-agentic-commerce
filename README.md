...
- [nohumans.directory](https://nohumans.directory) - Machine-first registry of paid x402 APIs with a three-tier verification ladder: continuous 402-conformance probes, paid verification with real USDC (550 endpoints bought from, 332 delivered — full outcome table and the SQL behind it at [/stats](https://nohumans.directory/stats)), and data-freshness checks that catch endpoints serving stale data. MCP server (in the official registry), REST discovery at `/v1/discover`, agent docs at `/llms.txt`, installable [agent skill](https://github.com/jalcodev/nohumans-directory-docs). Verification is free and ranking is never sold.
