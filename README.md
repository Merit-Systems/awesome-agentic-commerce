...
- [Solana Lens](https://autosophia.io) - Read-only Solana MCP server for trading/monitoring agents: token risk screening, wallet inspection, transaction simulation, priority-fee pricing. $0.01 USDC per call via x402, no account or API key.
- [Base Lens](https://autosophia.io) - Read-only Base MCP server for agents: ERC-20 risk screening with live Uniswap v3 slippage curves, wallet inspection, transaction simulation, gas pricing. $0.01 USDC per call via x402, no account or API key.
