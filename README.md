...
- [PulseNetwork](https://pulsenetwork.theaslangroupllc.com) - Vertical intelligence APIs for AI agents: 66 single-purpose x402 origins / 826 pay-per-call endpoints covering sanctions & OFAC screening, EU VAT validation, pre-trade token safety scans, SEC XBRL fundamentals, tax intelligence for 195 countries, climate, energy, health, and sports. Deterministic primitives from $0.01, USDC on Base; agent.json/OpenAPI/llms.txt on every origin. Also live on MPP (Tempo) via [MPP gateway](https://mpp.theaslangroupllc.com).
