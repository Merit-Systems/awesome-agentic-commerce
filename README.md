...
- [StillOS Notary](https://stillosdigitalholdings.com/notary/reference) - x402-paid claim verification, pre-transaction clearance, and compliance screening on Base mainnet: signed verdicts resolved against real external ground truth (GitHub PRs, Kalshi markets, HTTP probes), backed by an on-chain correctness bond slashable by anyone who proves a verdict wrong. `POST /notary/claim-verdict` ($0.05), `POST /notary/agent-clearance` (OFAC + on-chain wallet clearance, $0.25), `POST /notary/commit` ($0.10). Free tools: [domain trust-posture scanner](https://stillosdigitalholdings.com/notary/trust-score), 5 free verdicts/agent/day. [MCP server](https://www.npmjs.com/package/stillos-notary) available.
