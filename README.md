...
- [WickedAPI Trading Data API](https://api.wickedapi.com) - Crypto/stock market data for agents: real-time prices & OHLCV, indicators (ATR, Fair Value Gaps, ICT liquidity levels), perp funding/OI, Kalshi prediction markets, DeFi TVL, and an AI momentum-score verdict across 23 endpoints. x402 pay-per-call ($0.001/request, USDC on Base via Coinbase CDP) or a free API key; 5 endpoints (sentiment, macro calendar, gas, market overview, Kalshi probability) are free with no auth at all. [OpenAPI spec](https://api.wickedapi.com/openapi.json) · [llms.txt](https://api.wickedapi.com/llms.txt).
