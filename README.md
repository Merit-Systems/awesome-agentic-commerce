...
- [Liquid Agent](https://api.liquidagent.ai) - Agent-native tokenized-stock index on Base: mint your own on-chain ERC-4626 vault of Coinbase's tokenized NVDA/META/AAPL/GOOGL, buy from $1, set custom weights, rebalance, and exit in-kind or to USDC any block. No oracle (self-priced TWAP NAV), no project token. OpenAPI 3.1 + `/.well-known/x402` discovery (SIWX); reads are free and transact endpoints return unsigned calldata the agent signs and broadcasts with its own gas (server holds no key). [Guide](https://api.liquidagent.ai/v1/guide) · [Discovery](https://api.liquidagent.ai/.well-known/x402) · [Status](https://api.liquidagent.ai/v1/status) · [Code](https://github.com/LiquidAgent/liquidagentx402)
