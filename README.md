...
- [PulseFeed](https://pulsefeed.dev/status) - Independent measurement of x402 payment endpoints: check an endpoint before paying it (live figure for the dead share at pulsefeed.dev/status.json) — liveness (reads the challenge from both the response body and the v2 PAYMENT-REQUIRED header), observation-count-qualified uptime, anomaly scan, on-chain receiver check. Also a live ecosystem observatory, a daily "State of x402" report, an [observation feed](https://pulsefeed.dev/incidents) (receiver-address changes between observations, catalog price vs. challenge price mismatches, invalid receivers, testnet endpoints listed as production, schemes outside the x402 spec — each with an on-chain reference; measurements, not accusations of intent), and a cross-domain [Data API](https://pulsefeed.dev/data) (x402 + MCP trust data in one call). npm: [pulsefeed-x402-guard](https://www.npmjs.com/package/pulsefeed-x402-guard), [pulsefeed-x402-ai-tools](https://www.npmjs.com/package/pulsefeed-x402-ai-tools) (Vercel AI SDK + LangChain), [pulsefeed-x402-mcp](https://www.npmjs.com/package/pulsefeed-x402-mcp).
