...
- [tryx402](https://github.com/crypto-yannso/tryx402) - Budget router and execution gateway over x402 ("the OpenRouter for agent payments"): hard budget caps, idempotent paid calls, cost ledger, and multi-currency fiat billing with margin. Python SDK ([PyPI](https://pypi.org/project/tryx402/)), CLI, and MCP server (`gateway_search`, `gateway_discover`, `gateway_call`, `gateway_spent`) sharing one session-wide budget.
