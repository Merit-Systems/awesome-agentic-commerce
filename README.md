...
- [BIII](https://biii-production.up.railway.app/openapi.json) - Non-custodial safe-to-pay & token-genuineness verdicts on Base: is a token the genuine issuer's contract or a look-alike (impersonation), is a wallet known-bad (OFAC + community scam floor). Fail-closed and advisory, every verdict re-verifiable on-chain; paid per call via x402 ($0.03 USDC). [MCP server / source](https://github.com/philpof102-svg/biii) available.
