...
- [deposit.now](https://deposit.now) - Open x402 agent funding rail (USDC on Base). Agents POST `/api/deposit` to fund any EVM wallet or provision a managed child; pay amount + 1% via HTTP 402. [Docs](https://deposit.now/docs) � [OpenAPI](https://deposit.now/openapi.json) � [llms.txt](https://deposit.now/llms.txt).
