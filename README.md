...
- [PreFlight Checker](https://x402-api-catalog.onrender.com) – Pre-flight checks for autonomous agents, before they install a package, call a service, or trust a domain. Code checks: npm package trust/risk score (registry age, downloads, GitHub org/stars, OSV.dev vulnerabilities, $0.02), GitHub repo health ($0.02). Network checks: domain liveness ($0.02). Ops checks: audits another x402 endpoint for why aggregators mark it "down" and returns a concrete fix ($1.00). No API key, no signup.
