...
- [kadec0 x402 Scrapers](https://api.kadec0.xyz) - 30 pay-per-call data endpoints for AI agents across crypto/DeFi (DeFi TVL, stablecoin supply, yield pools, token prices, gas oracle), web data and scrapers (URL-to-markdown, SERP search, GitHub/Hacker News/Stack Overflow, WHOIS/DNS, company enrichment), and civic/health/academic feeds (SEC EDGAR, congressional trades, FDA, CVE, arXiv/PubMed). $0.005–$0.15 USDC per call on Base via the keyless Coinbase CDP facilitator, settling direct to wallet — no accounts, no signup. Crawlable discovery manifest at [/.well-known/x402](https://api.kadec0.xyz/.well-known/x402) and [OpenAPI](https://api.kadec0.xyz/openapi.json). ([Docs](https://api.kadec0.xyz/docs))
