...
- [Utilia Solana Preflight](https://utilia.ink) – Deterministic Solana transaction simulation, diagnostics, priority-fee, and SPL token-risk APIs plus bounded PDF and audio transforms, payable in USDC through x402 v2 at $0.002–$0.01 per call. The hosted MCP endpoint settles on Solana; explicit HTTP aliases also support Base. Includes [MCP](https://api.utilia.ink/mcp), [OpenAPI](https://api.utilia.ink/openapi.json), [x402 discovery](https://api.utilia.ink/.well-known/x402), and an [on-chain agent identity](https://8004market.io/agent/solana/mainnet-beta/1462).
