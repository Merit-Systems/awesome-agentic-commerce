...
- [Agent Trust API](https://agent-trust-api-496e.onrender.com) – npm-package trust/risk scoring via x402 ($0.02 USDC on Base): registry age, weekly downloads, GitHub org/stars, and OSV.dev vulnerabilities aggregated into one score with reasons. Built so an agent can check a package before running `npx`/installing it. No API key, no signup.
