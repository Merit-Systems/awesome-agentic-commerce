...
- [Agent402](https://agent402.tools) - Open-source x402 + MPP + MCP server - the applied layer of Agentic Finance (AIFI) - with a catalog of 500+ (400+ pay-per-call tools + 100+ skill packs) for AI agents: live web search + cited answers, headless browser, OCR/PDFs, finance/SEC/macro data, wallet-keyed memory, and an OpenAI-compatible LLM gateway. USDC on Base/Solana/Polygon/Arbitrum/Monad/Celo/Avalanche/Sei/Optimism/Stellar/Algorand + USDG on Robinhood Chain (12 rails), or free via proof-of-work. Also ships `agent402-tollbooth` (pay-per-crawl sell-side gate) and an open x402 Index + Smart Order Router. Hosted MCP at https://agent402.tools/mcp. ([GitHub](https://github.com/MikeyPetrillo/Agent402))
