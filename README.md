...
- [The Stall](https://github.com/thebrierfox/the-stall) - Remote, pay-per-call MCP server with 198 x402-monetized capabilities (stocks, forex, DeFi, crypto, AI research, geospatial, domain intel, audio transcription) on Base mainnet. Any x402-compatible agent pays per call in USDC — no API keys, no accounts required. ([GitHub](https://github.com/thebrierfox/the-stall) · [Agent card](https://the-stall.intuitek.ai/.well-known/agent.json))
