...
- [Stride20k Data Gateway](https://gateway.stride20k.com) - 17 pay-per-call data endpoints for agents — x402 exact scheme, USDC on Base, no accounts or API keys, settle-on-success (failed responses are never charged). Crypto prices, DeFi TVL, Uniswap pool state, gas, FX, US Treasury yields, US weather, WHOIS, DNS, email validation, web→markdown extraction, Hacker News, Wikipedia, npm/PyPI health, deterministic token-risk scoring, $1 Domain Dossier. Machine manifest at [/.well-known/x402.json](https://gateway.stride20k.com/.well-known/x402.json); [MCP client](https://github.com/Marysbrain/x402-gateway-mcp) with spend caps (`npm i x402-gateway-mcp`).
