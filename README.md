...
- [GoCreative](https://api.gocreativeai.com) - Compliance & regulated-data APIs for AI agents: OFAC/sanctions/PEP screening, KYB, SEC, FRED, FDA recalls, and federal awards. 330+ keyless pay-per-call tools (USDC on Base/Solana). [MCP server](https://api.gocreativeai.com/mcp).
- [OmniCall](https://omnicall.gocreativeai.com) - Agent-tools gateway: 248 LLMs plus image/video/voice/music generation and live crypto/DeFi/markets/web-search in one keyless, pay-per-call endpoint. [npm](https://www.npmjs.com/package/omnicall-mcp).
- [Prism](https://enrich.gocreativeai.com) - Data enrichment for AI agents: people-enrich (verified work email + phone from a name + company domain), company/domain enrichment, and email validation. Keyless, pay-per-call USDC. [MCP server](https://enrich.gocreativeai.com/mcp).
- [Predict](https://predict.gocreativeai.com) - Prediction-market data for AI agents: Polymarket + Kalshi market search, prices, odds, and order books by keyword. Keyless, pay-per-call USDC. [MCP server](https://predict.gocreativeai.com/mcp).
