...
- [NodeFlare](https://nodeflare.app/docs/x402) - Pay-per-request JSON-RPC on 23 EVM chains (Ethereum, Base, Arbitrum, plus young chains like Robinhood Chain, Plasma, Ink and Zircuit) served from own bare-metal nodes. USDC on Base, from $0.001/call priced by compute units, heavy methods (`eth_getLogs`, `trace_*`, `debug_*`) included, no account or key. All 23 endpoints listed in the CDP Bazaar.
