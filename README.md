...
- [Second Opinion](https://secondopinionx402.com) - Adversarial verification API for AI agents: an independent skeptic attempts to refute a claim before the agent acts on it, returning a structured verdict with calibrated confidence, citations, and an Ed25519-signed receipt. Pay-per-verdict via x402 ($0.03–$0.75 USDC) on Algorand, Base, and Solana; free trial in the browser (no wallet); no account or API key; zero-install hosted MCP connector for agent stacks; `pip install secondopinion-x402` for LangChain/CrewAI. ([MCP](https://secondopinionx402.com/mcp) | [Benchmark](https://secondopinionx402.com/benchmark))
