...
- [Agent402](https://agent402.tools) - Open-source, self-hostable x402 + MCP server: ~1,100 deterministic web tools (browser, search, PDFs, images, live data, identifiers, conversions) priced from $0.001 USDC on Base. Free tier via proof-of-work for pure-CPU tools. Ships a Smart Order Router (`/api/route`) and on-chain leaderboard (`/api/leaderboard`) that rank across every seller in the Coinbase CDP Bazaar, plus `agent402-tollbooth` (pay-per-crawl middleware) and `agent402-client` (buyer SDK). ([GitHub](https://github.com/MikeyPetrillo/Agent402), [MCP](https://agent402.tools/mcp))
