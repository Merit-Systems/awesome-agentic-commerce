...
- [kadec0 Research API](https://api.kadec0.xyz) — Pay-per-call academic research API for AI agents: multi-source paper search (arXiv, PubMed, OpenAlex, Crossref, Semantic Scholar), open-access full-text & PDF retrieval, citation lookup (references + cited-by), related-papers / literature review, and author search. Part of a broader keyless x402 data service (also crypto/DeFi, web, and civic data endpoints). $0.02–$0.04 USDC per call on Base via the Coinbase CDP facilitator — no accounts or signup. ([Discovery](https://api.kadec0.xyz/.well-known/x402)) ([OpenAPI](https://api.kadec0.xyz/openapi.json)) ([Docs](https://api.kadec0.xyz/docs))
