...
- [Link Preview Cache API](https://link-preview-cache-api.onrender.com) - Open Graph/link preview metadata for AI agents, served from a shared cache so most calls cost a third of a fresh-fetch service ($0.003 vs $0.01). $0.003 USDC per call on Base mainnet via Coinbase CDP facilitator, no API keys. Also available as an MCP tool.
