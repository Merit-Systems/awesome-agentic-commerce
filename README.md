...
- [SYNTHORA Contract Guard](https://pay.hergertsynthora.com/service) - On-chain contract safety check for AI agents: honeypot, rug-pull, hidden-mint and owner-privilege detection on Base. Deterministic engine; every response carries a verifiable Ed25519-signed verdict with on-chain data provenance, so an agent can decide whether a contract is safe before it interacts. `POST {"input":"0x<address>"}`, $0.005 USDC on Base via the Coinbase CDP facilitator, no signup. ([OpenAPI](https://pay.hergertsynthora.com/openapi.json) · [Discovery](https://pay.hergertsynthora.com/.well-known/x402))
- [SYNTHORA x402 Registry Watch](https://regwatch.hergertsynthora.com/service) - Signed Ed25519 delta feed of the live x402/M2M market: endpoint, payTo, listing and price changes for router/discovery agents. Each row and the whole response carry an Ed25519 provenance receipt. First 5 calls free per Base wallet, then $0.02 USDC on Base via the Coinbase CDP facilitator. ([OpenAPI](https://regwatch.hergertsynthora.com/openapi.json) · [Discovery](https://regwatch.hergertsynthora.com/.well-known/x402))
