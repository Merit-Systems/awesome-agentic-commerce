...
- [Truth Bear (GAUGE)](https://truthbear.co) - 180+ official-source data signals for AI agents (USGS/NOAA/EPA/EIA/SEC EDGAR/IMF PortWatch). Pay-per-call USDC on Base + Solana, from $0.005, no API key, no signup, MCP-native, no-data-no-charge. Every paid record ships a canonical sha256 record_hash you recompute offline with our open verifier — screening-level, not decision-grade; no forecasts or advice. [MCP server](https://www.npmjs.com/package/mcp-gauge-x402) available.
