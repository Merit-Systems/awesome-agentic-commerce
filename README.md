...
- [StillOS Notary](https://nolawealthfinancial.com/notary/reference) - x402-paid claim verification on Base mainnet: signed verdicts resolved against real external ground truth (GitHub PRs, Kalshi markets, HTTP probes), backed by an on-chain correctness bond slashable by anyone who proves a verdict wrong. Free tier (5/agent/day), `POST /notary/claim-verdict` ($0.25), `POST /notary/commit` ($0.10). [MCP server](https://www.npmjs.com/package/stillos-notary) available.
