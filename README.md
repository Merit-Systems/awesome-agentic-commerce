...
- [zaref](https://zaref.dev) - South African reference data API for AI agents: interest rates (SARB repo/prime), ZARONIA, fuel prices, SARS tax tables, FX, bond yields, wages and grants as cited JSON with effective dates and staleness flags, plus calculators. Pay-per-call USDC on Base via x402; catalog, public holidays and the VAT calculator are free, no API key. Python client: `pip install zaref`. ([Python SDK](https://pypi.org/project/zaref/)) ([GitHub](https://github.com/SpookySats/zaref-python)) ([OpenAPI](https://zaref.dev/openapi.json)) ([llms.txt](https://zaref.dev/llms.txt))
- [verza](https://verza.dev) - South African KYB and verification primitives for AI agents: ID-number validation, VAT-number validation, and NCA loan-agreement lawfulness checks against the Reg 42 interest caps. Pay-per-call USDC on Base via x402, no accounts. ([OpenAPI](https://verza.dev/openapi.json)) ([llms.txt](https://verza.dev/llms.txt))
- [jurat](https://jurat.dev) - Evidence infrastructure for AI agents: notarized snapshots of any URL, sports finals with proof attached, watchers and wakeups for ephemeral agents, and x402 pre-flight, which runs 18 conformance checks on any other x402 endpoint before you pay it. Pay-per-call USDC on Base via x402, no signup. ([OpenAPI](https://jurat.dev/openapi.json)) ([llms.txt](https://jurat.dev/llms.txt))
