...
- [Agent402](https://agent402.tools) - Open-source, self-hostable x402 + MCP server: 500+ deterministic web tools + 100 skill packs (browser, search, OCR, PDFs, live finance/crypto/macro data, SEC EDGAR, wallet-keyed memory) from $0.001/call — USDC on Base, Solana, Polygon, Arbitrum, Monad, Celo, Avalanche, Stellar, Algorand; USDG on Robinhood Chain (10 chains). Free tier via proof-of-work for pure-CPU tools. Ships a Smart Order Router (`/api/route`) and on-chain leaderboard ranking every seller in the Coinbase CDP Bazaar, plus `agent402-tollbooth` (pay-per-crawl middleware) and `agent402-client` (buyer SDK). ([GitHub](https://github.com/MikeyPetrillo/Agent402), [MCP](https://agent402.tools/mcp))
